...
function evalJS(b, player) {
  ...
  for(const[dr,dc]of dirs)for(let r=0;r<ROWS;r++)for(let c=0;c<COLS;c++){
    ...
    if(cnt===5)sc+=1000000;else if(cnt===4&&emp===1)sc+=8000;else if(cnt===3&&emp===2)sc+=500;else if(cnt===2&&emp===3)sc+=50;
    if(ocnt===4&&emp===1)sc-=12000;else if(ocnt===3&&emp===2)sc-=800;else if(ocnt===2&&emp===3)sc-=60;
  }
  const center=4;
  for(let r=0;r<ROWS;r++){if(b[r][center]===player)sc+=4;if(b[r][center-1]===player)sc+=2;if(b[r][center+1]===player)sc+=2;}
  return sc;
}
...
function minimaxJS(b, depth, alpha, beta, maximizing, aiPlayer) {
  ...
  if(isFullJS(b)||depth===0){const opp=aiPlayer===1?2:1;return evalJS(b,aiPlayer)-evalJS(b,opp);}
  const opp=aiPlayer===1?2:1;
  if(maximizing){
    let best=-Infinity;
    for(const col of COL_ORDER){const r=getDropRowJS(b,col);if(r<0)continue;b[r][col]=aiPlayer;const sc=minimaxJS(b,depth-1,alpha,beta,false,aiPlayer);b[r][col]=0;if(sc>best)best=sc;if(best>alpha)alpha=best;if(beta<=alpha)break;}
    return best;
  }else{
    let best=Infinity;
    for(const col of COL_ORDER){const r=getDropRowJS(b,col);if(r<0)continue;b[r][col]=opp;const sc=minimaxJS(b,depth-1,alpha,beta,true,aiPlayer);b[r][col]=0;if(sc<best)best=sc;if(best<beta)beta=best;if(beta<=alpha)break;}
    return best;
  }
}
...
function getBestColJS(board, aiPlayer) {
  ...
  for (let depth = 2; depth <= 10; depth++) {
    ...
    if(Date.now()<jsDeadline)bestCol=iterCol;
  }
  return bestCol;
}
